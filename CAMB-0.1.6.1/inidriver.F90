	...
	call NLCDMShapes_ReadParams2(DefIni, SD)
	!write(*,*) 'SD%num_k is ',SD%num_k
	!if(use_nlcdm_shape)then
	!	write(*,*) 'This if statement works'
	!	do i=1,600
	!		write(*,*) 'SD%shapepower is ',SD%shapepower(i)
	!	enddo 	
	!endif	
    ...
    if (P%PK_WantTransfer) then
		...
		!!! 20th July 2018; Added here an if statement conditional on shape modification flag
		if(.not. use_nlcdm_shape)then
	        call Transfer_SaveMatterPower(MT,MatterPowerFileNames)
		else
			write(*,*)'Using NLCDM shape modification...'
			call Transfer_SaveMatterPower_Shape(SD,MT,MatterPowerFileNames)
		endif
...
100 stop 'Must give num_massive number of integer physical neutrinos for each eigenstate'
	...
	subroutine NLCDMShapes_ReadParams2(Ini, SD)
	use IniFile
	Type(TIniFile) :: Ini
	character(LEN=Ini_max_string_len) shapefile
    integer i,nz
	!Type(ShapeData) :: SD
	Type(ShapeData), intent(inout) :: SD

	!nz = 1 !P%transfer%PK_num_redshifts	
    use_nlcdm_shape = Ini_Read_Logical_File(Ini,'use_nlcdm_shape',.false.)
    if(.not. use_nlcdm_shape)then
		write(*,*)'Not using NLCDM shape modification'
	else
		write(*,*)'Using NLCDM shape modification: Reading in shape.'
		shapefile = Ini_Read_String_File(Ini,'shapefile')
		open(unit=10,file=shapefile,status='old')
		nshape_data=0
		do i=1,nsmax+1
			read(10,*,end=100)kh_data(i),shape_data(i)
			kh_data(i)=dlog(kh_data(i))
			nshape_data=nshape_data+1
		enddo	

		stop
100     close(10)
        write(*,'("read in ", I8, " (k, shape) data points from ", a)') nshape_data, trim(shapefile)

		!write(*,*) '1. nshape_data is ',nshape_data
    	SD%num_k = nshape_data
    	!SD%num_z = nz
		...
		SD%shapepower = shape_data
		SD%log_kh = kh_data
		call spline_shapes(SD)

		! Checked this all works now,
		!do i=1,600
			!write(*,*) 'shape_data is ',shape_data(i)
			!write(*,*) 'SD%shapepower is ',SD%shapepower(i)
			!write(*,*) 'SD%log_kh is ',SD%log_kh(i)
		!enddo 
		
    endif
	end subroutine NLCDMShapes_ReadParams2
