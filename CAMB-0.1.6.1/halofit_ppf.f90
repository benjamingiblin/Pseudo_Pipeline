    ...
    ELSE
        ...
        do itf = 1, CAMB_Pk%num_z
            ...
            do
				...
				!write(*,*) 'and now sig is ', sig  	
                diff=sig-1.0
				!write(*,*) 'rmid and diff are... ', rmid, ' ', diff
                if (abs(diff).le.0.001) then
					!write(*,*) '1. This one activated as rmid and diff are... ', rmid, ' ', diff
                    rknl=1./rmid
                    rneff=-3-d1
                    rncur=-d2
                    exit
                elseif (diff.gt.0.001) then
					!write(*,*) '2. This one activated as rmid and diff are... ', rmid, ' ', diff
                    xlogr1=log10(rmid)
                elseif (diff.lt.-0.001) then
					!write(*,*) '3. This one activated as rmid and diff are... ', rmid, ' ', diff
                    xlogr2=log10(rmid)
                endif
				!write(*,*) 'xlogr2 is... ', xlogr2		! If Error in halofit raised it's coz xlogr2 is never changed from 3.5
														! In cases where it's not raised, xlogr2 goes through 5 loops, changing from 0.75 to 0.567...
                if (xlogr2 < -1.9999) then
                    !is still linear, exit
                    goto 101
                else if (xlogr1>3.4999) then		!!! Alex Hall found a bug here: said xlogr2 originally; it needs to be xlogr1! Lower limit shouldn't go all the way up to 3.5: initial val of upper lim.
			...
			!write(*,*) 'IF Error in halofit is raised, this write statement and next do loop will be SKIPPED.' 
...
101         continue
			!do i=1, CAMB_PK%num_k
			!	write(*,*) 'CAMB_Pk%nonlin_ratio(i,itf) is ...', CAMB_Pk%nonlin_ratio(i,itf) 
			!end do
