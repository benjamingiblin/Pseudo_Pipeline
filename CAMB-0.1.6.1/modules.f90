	...
	!!! 20th July 2018; These variables were added for the nlcdm_shape modifications
	logical :: use_nlcdm_shape = .false.
	integer, parameter :: nsmax = 5000
	real(dl) kh_data(nsmax), shape_data(nsmax), ddshape_data(nsmax) 
	integer :: nshape_data
	private ddshape_data
	!!! 20th July 2018; These variables were added for the nlcdm_shape modifications
	...
	!!! 20th July 2018; The Type for reading in & interpolating the NLCDM Shapes !!!
	Type ShapeData
        ...
        !matpower is log(P_k)... what will shapepower be?
        real(dl), dimension(:), allocatable :: shapepower, ddmat
	...
	!!! 20th July 2018; The subroutines/functions for reading in & interpolating the NLCDM Shapes !!!



	subroutine spline_shapes(SD)
	Type(ShapeData), intent(inout) :: SD
    real(dl), parameter :: khlo=1.d30, khhi=1.d30
    !call spline(kh_data,shape_data,nshape_data,khlo,khhi,ddshape_data) 
	call spline(SD%log_kh,SD%shapepower,SD%num_k,khlo,khhi,SD%ddmat) 
    end subroutine spline_shapes
...
500     if (L< lmax_extrap_highl) &
    ...
    do zix=1, CP%Transfer%PK_num_redshifts
		...
		!write(*,*) ' Giblin you value is:', PK(:,zix)	! This is not being activated.
    ...
    do ik=1,MTrans%num_q_trans
        ...
        do itf = 1, nz
            PK_data%matpower(ik,itf) = &
                ...
                *pi*twopi*h**3*power)
			!write(*,*) 'PK_data%matpower(ik,itf) is ', PK_data%matpower(ik,itf) ! checked and log(matpower(ik)) ~ 7 here.
        end do
    end do
   ...
   subroutine Transfer_GetMatterPowerData_Shape(SD, MTrans, PK_data, power_ix, itf_only, var1, var2)
    !Does *NOT* include non-linear corrections
    !Get total matter power spectrum in units of (h Mpc^{-1})^3 ready for interpolation.
    !Here there definition is < Delta^2(x) > = 1/(2 pi)^3 int d^3k P_k(k)
    !We are assuming that Cls are generated so any baryonic wiggles are well sampled and that matter power
    ...
    !allocate(SD%shapepower(SD%num_k))
    !allocate(SD%ddmat(SD%num_k))
    !allocate(SD%nonlin_ratio(SD%num_k,nz))
    !allocate(SD%log_kh(SD%num_k))
    !allocate(SD%redshifts(nz))
    ...
    do ik=1,MTrans%num_q_trans
        ...
        do itf = 1, nz
			!write(*,*) 'ShapeData_k is ...', kh, ShapeData_k(SD,kh,itf)
            PK_data%matpower(ik,itf) = &
                log(ShapeData_k(SD,kh,itf)*MTrans%TransferData(s1,ik,itf_start+itf-1)*&
                MTrans%TransferData(s2,ik,itf_start+itf-1)*k &
                *pi*twopi*h**3*power)
				!write(*,*) 'PK_data%matpower(ik,itf) is ', PK_data%matpower(ik,itf)	! checked and log(matpower(ik)) ~ 7 here.
        end do
    ...
    end subroutine Transfer_GetMatterPowerData_Shape
    ...
    do i = 1,PK_Data%num_z
		...
		write(*,*) 'PK_data%matpower(1,i) ',PK_data%matpower(1,i) ! checked and PK_data%matpower(1,i) ~ 5
    end do
	...
	!write(*,*) 'Giblin you value is:', PK_data%matpower		! this one is not being activated.
    call MatterPowerdata_getsplines(PK_data)
	...
	!write(*,*) 'ShapeData has this many points: ',SD%num_k
	!write(*,*) 'ShapeData shapepower looks like ',SD%shapepower
    logk = log(kh)
	!write(*,*) 'logk is... ',logk    
    ...
    function ShapeData_k(SD,  kh, itf) result(outpower)
    !Get matter power spectrum at particular k/h by interpolation
    Type(ShapeData) :: SD
    integer, intent(in) :: itf
    real (dl), intent(in) :: kh
    ...
    logk = log(kh)
	!write(*,*) 'logk is... ',logk
    if (logk < SD%log_kh(1)) then
        dp = (SD%shapepower(2) -  SD%shapepower(1)) / &
            ( SD%log_kh(2)-SD%log_kh(1) )
        outpower = SD%shapepower(1) + dp*(logk - SD%log_kh(1))
		!write(*,*) 'Are we extrapolating?'
    else if (logk > SD%log_kh(SD%num_k)) then
        !Do dodgy linear extrapolation on assumption accuracy of result won't matter

        dp = (SD%shapepower(SD%num_k) -  SD%shapepower(SD%num_k-1)) / &
            ( SD%log_kh(SD%num_k)-SD%log_kh(SD%num_k-1) )
        outpower = SD%shapepower(SD%num_k) + dp*(logk - SD%log_kh(SD%num_k))
    else
		!write(*,*) 'Does it break here?'
        llo=min(i_last,SD%num_k)
        do while (SD%log_kh(llo) > logk)
			!write(*,*) ' Here...?'            
			llo=llo-1
        end do
        do while (SD%log_kh(llo+1)< logk)
			!write(*,*) ' Or here...?'  
            llo=llo+1
        end do
        i_last =llo
        lhi=llo+1
        ho=SD%log_kh(lhi)-SD%log_kh(llo)
        a0=(SD%log_kh(lhi)-logk)/ho
        b0=1-a0
		!write(*,*) 'Does it break here?'
        outpower = a0*SD%shapepower(llo)+ b0*SD%shapepower(lhi)+&
            ((a0**3-a0)* SD%ddmat(llo) &
            +(b0**3-b0)*SD%ddmat(lhi))*ho**2/6
    end if

  

    end function ShapeData_k
    ...
    subroutine Transfer_GetMatterPowerS_Shape(SD, MTrans,outpower, itf, in, minkh, dlnkh, npoints, var1, var2)
    Type(MatterTransferData), intent(in) :: MTrans
	Type(ShapeData), intent(in) :: SD
	integer :: i
    integer, intent(in) :: itf, in, npoints
    integer, intent(in), optional :: var1, var2
    real, intent(out) :: outpower(*)
    ...
    minkhd = minkh; dlnkhd = dlnkh
    call Transfer_GetMatterPowerD_Shape(SD,MTrans,outpowerd, itf, in, minkhd, dlnkhd, npoints,var1, var2)
    outpower(1:npoints) = outpowerd(1:npoints)

    end subroutine Transfer_GetMatterPowerS_Shape
    ...
    if (CP%NonLinear/=NonLinear_none .and. CP%NonLinear/=NonLinear_Lens) then
		!write (*,*) '2. Is this thing being activated???? ' ! YES IT IS!		
    ...
    do ik=1,MTrans%num_q_trans
		...
		!write(*,*) 'matpower(ik) is ', matpower(ik)		! checked and log(matpower(ik)) ~ 20 here.
    ...
    subroutine Transfer_GetMatterPowerD_Shape(SD,MTrans,outpower, itf_PK, in, minkh, dlnkh, npoints, var1, var2)
    !Allows for non-smooth priordial spectra
    !if CP%Nonlinear/ = NonLinear_none includes non-linear evolution
    !Get total matter power spectrum at logarithmically equal intervals dlnkh of k/h starting at minkh
    !in units of (h Mpc^{-1})^3.
    ...
    if (CP%NonLinear/=NonLinear_none .and. CP%NonLinear/=NonLinear_Lens) then
		!write (*,*) '2. Is this thing being activated???? ' ! YES IT IS!
		call Transfer_GetMatterPowerData_Shape(SD, MTrans, PK, in, itf) ! Mar 16, changed to use default variable
        call NonLinear_GetRatios(PK)
    end if
    ...
    do ik=1,MTrans%num_q_trans
        ...
        if (CP%NonLinear/=NonLinear_none .and. CP%NonLinear/=NonLinear_Lens) &
            atransfer = atransfer* PK%nonlin_ratio(ik,1)**2 !only one element, this itf
        matpower(ik) = log(ShapeData_k(SD,kh,itf)*atransfer*k*pi*twopi*h**3)
		!write(*,*) 'matpower(ik) is ', matpower(ik)		! checked and log(matpower(ik)) ~ 20 here.
		!write(*,*) 'ShapeData_k(SD,kh,itf) is ', ShapeData_k(SD,kh,itf)
		!write(*,*) ShapeData_k(SD,kh,itf)
		!write(*,*) kh
        !Put in power spectrum later: transfer functions should be smooth, initial power may not be
    end do
    ...
    end subroutine Transfer_GetMatterPowerD_Shape
	...
	!write (*,*) '3. Is this thing being activated???? ' ! No it is not.
    ...
    do ik=1, MTrans%num_q_trans + 2
        ...
        do subk = 1, nsub
			...
			write(*,*) 'Is this guy getting activated?' !No it doesn't.
    ...
    do itf=1, CP%Transfer%PK_num_redshifts
        if (FileNames(itf) /= '') then
            if (.not. transfer_interp_matterpower ) then
                ...
                do in = 1, CP%InitPower%nn
					! write (*,*) '4. Is this thing being activated???? ' ! No it is not.
    ...
    subroutine Transfer_SaveMatterPower_Shape(SD, MTrans, FileNames)
    use IniFile
    !Export files of total  matter power spectra in h^{-1} Mpc units, against k/h.
    Type(MatterTransferData), intent(in) :: MTrans
	Type(ShapeData), intent(in) :: SD
    ...
    do itf=1, CP%Transfer%PK_num_redshifts
        if (FileNames(itf) /= '') then
            if (.not. transfer_interp_matterpower ) then
                ...
                do in = 1, CP%InitPower%nn
					! write (*,*) '4. Is this thing being activated???? ' ! No it is not.
					! Maybe alter nontheless
                    call Transfer_GetMatterPowerData_Shape(SD, MTrans, PK_data, in, itf_PK)
                    !JD 08/13 for nonlinear lensing of CMB + LSS compatibility
                    !Changed (CP%NonLinear/=NonLinear_None) to CP%NonLinear/=NonLinear_none .and. CP%NonLinear/=NonLinear_Lens)
                    if(CP%NonLinear/=NonLinear_none .and. CP%NonLinear/=NonLinear_Lens)&
                        call MatterPowerdata_MakeNonlinear(PK_Data)
                ...
                allocate(outpower(points,CP%InitPower%nn,1))
                do in = 1, CP%InitPower%nn
                    call Transfer_GetMatterPowerS_Shape(SD,MTrans,outpower(1,in,1), itf, in, minkh,dlnkh, points)
                end do
    ...
    end subroutine Transfer_SaveMatterPower_Shape
