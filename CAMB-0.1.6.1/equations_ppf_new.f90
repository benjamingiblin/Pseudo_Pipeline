    ...
    integer, parameter :: nwmax = 5000, nde = 2000, nshape = 600
    integer :: nw_ppf
    real(dl) w_ppf(nwmax), a_ppf(nwmax), ddw_ppf(nwmax) !, k_ppf(nwmax), shape_ppf(nwmax) , ddshape_ppf(nwmax)
    real(dl) rde(nde),ade(nde),ddrde(nde) !,kshape(nshape),sshape(nshape),ddshape(nshape) 
    real(dl), parameter :: amin = 1.d-9
	!real(dl), parameter :: kmin = 1.d-5
    logical :: is_cosmological_constant
    private nde,ddw_ppf,rde,ade,ddrde,amin !ddshape_ppf

    contains

	!NLCDMShapes subroutine used to be here.
...
100     close(10)
	...
	!subroutine setddkshape
	!real(dl), parameter :: shapelo=1.d30, shapehi=1.d30
	!!write(*,*)'shapelo is ',shapelo
	!!write(*,*)'shapehi is ',shapehi
	!call spline(k_ppf,shape_ppf,nw_ppf,shapelo,shapehi,ddshape_ppf) !a_ppf is lna here
	!end subroutine setddkshape
	...
	! amin->kmin, almin->klmin, atot->ktot, nde->nshape, ade->kshape, rde->sshape, ddrde->ddshape
	! rombint integrates drdlna_de from al to 0._dl, 
	! ...atol specified the relative accuracy of the integral
	! drdlna_de returns dr/dlna, which seems analytical; but what is r?
    !subroutine interpolshape
    !real(dl), parameter :: rlo=1.d30, rhi=1.d30
    !real(dl) :: ktol, klmin, kl, rombint, fint
    !integer :: i
    !external rombint
    !ktol=1.d-5 !relative accuracy of rombint intergral
    !klmin=dlog(kmin)
    !do i=1,nshape
    !    kl=klmin-klmin/(nshape-1)*(i-1)    !interpolate between amin and today
    !    fint=rombint(drdlna_de, kl, 0._dl, ktol)+4._dl*kl  !!! ????? HOW TO CHANGE THIS LINE...?
    !    kshape(i)=kl
    !    sshape(i)=dexp(fint) !rho_de*a^4 normalize to its value at today
    !enddo
    !call spline(kshape,sshape,nshape,rlo,rhi,ddshape)
    !end subroutine interpolshape
